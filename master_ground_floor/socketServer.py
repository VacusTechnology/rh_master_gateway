...
RECEIVE_BUFFER = 5000
SOCKET_BINDING_ADDR=('127.0.0.1',12300)
    ...
    try:
        ...
        if len(jsonList)!=0:
            for elem in jsonList:
                sql = """SELECT * FROM ground_floor WHERE macaddress = %s"""
                val = (elem["macaddress"],)
                cursor = db.cursor() 
                ...
                if result==None:
                    ...
                    sql = """INSERT INTO ground_floor (macaddress,X,Y,temp,humidity,airflow,iaq,alert,battery,packet,slave) VALUES (%s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
                    ...
                    if elem["packet"]>result[10]:
                        ...
                        sql="""UPDATE ground_floor SET X=%s,Y=%s,temp=%s,humidity=%s,airflow=%s,iaq=%s,alert=%s,battery=%s,packet=%s,slave=%s WHERE macaddress = %s"""
